    ...
    return (
        <section>
            <div className='flex justify-between items-center m-10'>
                ...
                <button onClick={() => setModal(true)} className='border p-3 font-semibold rounded-lg bg-gray-100'>Add Service +</button>
            </div>
            <div className="flex gap-5 mx-10 ">
                {serviceData?.map((service: ServiceData) => (
                    <div key={service.id} className="border p-4 rounded-lg shadow-md w-72 h-fit">
                        ...
                        <p className="text-gray-500">{service.ConnectedService.length} service{service.ConnectedService.length > 1 ? 's' : ''} available</p>
                            ...
                            <div className="mt-4 space-y-2">
                                {service.ConnectedService.map((detail) => (
                                    <div key={detail.id} className="border-t pt-2">
                                        ...
                                        {detail.additionalOffer && <p className="text-gray-600">Additional Offer: {detail.additionalOffer}</p>}
                                    </div>
                                ))}
                            </div>
                        )}
