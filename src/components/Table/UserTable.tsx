...
import { useAllUsersQuery, useUserStatusUpdateMutation } from '@/Redux/Api/userApi';
...
const UserTable = () => {
    ...
    const { userData, isLoading, totalPages } = useAllUsersQuery({ limit, page }, {
                        ...
                        <tbody>
                            {
                                userData?.map((item: UserInterFace, index: number) => (
                                    <motion.tr initial={{ y: 100 * (index + 1), opacity: 0 }} animate={{ y: 0, opacity: 1, transition: { duration: 0.5 } }} key={index} className="border-b text-center">
                                        ...
                                        <td><Image src={item.
                                            profileImage ? item.profileImage : noface} width={50} height={50} alt={item.userName} className='rounded-full mx-auto' ></Image></td>
